...
class ccRTP_Hello_Tx : public Thread, public TimerPort
{
  ...
  private:
	...
	uint32 ssrc;
	int RECEIVER_BASE;	//= 33628;
	int TRANSMITTER_BASE; //= 32520;
...
int main()
{
	...
	transmitter->start();
	cout << "Zmodyfikuj stan czujnika zblizeniowego aby otworzyc/zamknac brame.\nBrama zamknieta. Wcisnij 1 aby otworzyc" << endl;
	int prevStat = 0;
	for (;;)
	{
		...
		{
			salute[0] = '0';
			cout << "Brama zamknieta. Wcisnij 1 aby otworzyc" << endl;
		}
		else if (status == 1 && status != prevStat)
		{
			salute[0] = '1';
			cout << "Brama otwarta. Wcisnij 0 aby zamknac" << endl;
		}
		prevStat = status;
	}
