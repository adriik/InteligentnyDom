  ...
  public:
	ccRTP_Hello_Rx(InetHostAddress localIP, InetHostAddress destinationIp, int PortRx, int PortTx)
	{
		...
		if (!destination_ip)
		{
			...
			zapiszLog("Bledny Destination IP");
	...
	void run(void)
	{
		...
		if (!socket->addDestination(destination_ip, TRANSMITTER_BASE))
		{
			...
			snprintf(buforDlaLogu, sizeof(buforDlaLogu), "Nie mozna polaczyc sie z portem: %d", TRANSMITTER_BASE);
			zapiszLog(buforDlaLogu);
			cerr << "Nie mozna polaczyc sie z portem: " << dec << TRANSMITTER_BASE;
		}
		...
		for (;;)
		{
			...
			{
				cout << "Rolety opuszczone" << endl;
			}
			else if (actualState != tmp && actualState == '1')
			{
				cout << "Rolety podniesione" << endl;
			}
			tmp = actualState;
			delete adu;
		}
