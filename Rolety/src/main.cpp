	...
	void run(void){
		...
		for( int i = 0 ; true ; i++ ){
			...
			while ( NULL == adu ) {
				...
				/*tutaj mozna zrobic wypisywanie obecnego stanu rolet (aduState)
				bez puszczania komunikatow o tym samym stanie
				int i, i_max=10, wyzej sleep(100), deklaracja przed while:
				const AppDataUnit aduState = NULL;
				*/
			}
			/*Tutaj jebnac aduState=adu, i wezej w petli wyswietlanie jak nizej adu->get
			*/
...
int main(int argc, char *argv[])
{
	...
	cin.get(); //TO jest do wyjebania moim zdaniem MS
