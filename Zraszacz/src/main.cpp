...
#define NAZWA_LOGU "Zraszacz_log"
...
unsigned char *status = &status_zraszacz;
...
class ccRTP_Hello_Rx : public Thread
{
  ...
  private:
	...
	uint32 ssrc;
	int RECEIVER_BASE;	//= 33622;
	int TRANSMITTER_BASE; //= 32514;
	...
	void run(void)
	{
		...
		for (int i = 0; true; i++)
		{
			...
			*status = ((unsigned char *)(adu->getData()))[0];
			...
			{
				cout << "Zraszacze wylaczone" << endl;
			}
			else if (actualState != tmp && actualState == '1')
			{
				cout << "Zraszacze wlaczone" << endl;
			}
			tmp = actualState;
			delete adu;
		}
...
class ccRTP_Hello_Tx : public Thread, public TimerPort
{
  ...
  private:
	...
	// identifier of this sender
	uint32 ssrc;
	int RECEIVER_BASE_CW;
	int TRANSMITTER_BASE_CW;
  ...
  public:
	ccRTP_Hello_Tx(InetHostAddress localIP, InetHostAddress destinationIp, int PortRx, int PortTx)
	{
		...
		local_ip = localIP;
		destination_ip = destinationIp;
		RECEIVER_BASE_CW = PortRx;
		TRANSMITTER_BASE_CW = PortTx;
		...
		socket = new RTPSession(local_ip, TRANSMITTER_BASE_CW);
		ssrc = socket->getLocalSSRC();
	}
	...
	void run(void)
	{
		...
		socket->setSchedulingTimeout(20000);
		socket->setExpireTimeout(3000000);
		if (!socket->addDestination(destination_ip, RECEIVER_BASE_CW))
			cerr << "Tx (" << hex << (int)ssrc
				 << "): could not connect to port."
				 << dec << RECEIVER_BASE_CW;
...
int main()
{
	...
	ccRTP_Hello_Tx *transmitter = new ccRTP_Hello_Tx(
		...
		doc.child("Dom").child("Zraszacz").child("PortRxCW").attribute("Wartosc").as_int(),
		doc.child("Dom").child("Zraszacz").child("PortTxCW").attribute("Wartosc").as_int());
